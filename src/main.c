        ...
        switch (opt) {
            case 'm':
                if(strcmp(optarg, "offset") == 0)
                    ...
                    printf("-m --fill_mod: offset / index\n");
                    exit(0);
                }
                break;
            case 'p':
...
int reduction(char* dex_path, char* item_path, int fill_mod, int pad_switch) {
    // if(argc != 4){
    //     printf("%s -[idx/off] -[-pad-on/-pad-off] <dex_path> <item_path>\n", args[0]);
    //     return 0;
    // }
    // int mod = 0;
    // int padding = 0;
    // if(strcmp(args[1], "-off") == 0) mod = 0;
    // else if(strcmp(args[1], "-idx") == 0) mod = 1;
    // char* dex_path = args[2];
    // char* item_path = args[3];
    int padding_len = 0;
    if(pad_switch == 1)
